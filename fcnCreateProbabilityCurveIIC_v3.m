function [yf,p,b,f]=fcnCreateProbabilityCurveIIC_v3(data,featureName,featureName2); 
...
eval(['x2 = data.' featureName 'freqmaxfactor;']); % 1, 1.5, 2, 2.5, 3, nan          
...
[yf,b,p] = fcnGetProbabilityCurves_v3(XX,YY)
...
th = max(yf); c0 = [1 0 0]; c1 = [1 1 0]; c = c0*th+(1-th)*c1; % line color 
figure(1); hold on; plot(f,yf,'color',c,'linewidth',2); 
